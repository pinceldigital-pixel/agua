// Centro dentro del donut + deshacer + notificaciones
const store = { get(){ return JSON.parse(localStorage.getItem('agua:min')||'{}'); }, set(v){ localStorage.setItem('agua:min', JSON.stringify(v)); } };
const todayKey = () => new Date().toISOString().slice(0,10);
const els = {
  arc: document.getElementById('arc'), pct: document.getElementById('pct'),
  nowMl: document.getElementById('nowMl'), goalMl: document.getElementById('goalMl'),
  amount: document.getElementById('amount'), add: document.getElementById('add'),
  undo: document.getElementById('undo'), settings: document.getElementById('settings'),
  openSettings: document.getElementById('openSettings'), goalInput: document.getElementById('goalInput'),
  intervalInput: document.getElementById('intervalInput'), startInput: document.getElementById('startInput'),
  endInput: document.getElementById('endInput'), save: document.getElementById('save'),
...
// Circunferencia (r=104 -> 2πr ≈ 653.45)
...
function updateUndoState(){ els.undo.disabled = (state.logs[todayKey()]||[]).length===0; }
function render(){
...
// Long press para repetir
let pressTimer=null;
els.add.addEventListener('pointerdown', ()=>{ pressTimer=setInterval(()=> add(parseInt(els.amount.value,10)), 400);});
['pointerup','pointerleave','pointercancel'].forEach(ev=> els.add.addEventListener(ev, ()=>{ if(pressTimer) clearInterval(pressTimer); pressTimer=null; }));
...
els.save.addEventListener('click', async ()=>{
  ...
  persist(); render(); await ensureNotificationPermission(); scheduleReminders(); els.settings.close();
});
...
let timer=null; function scheduleReminders(){ if(timer) clearInterval(timer); timer=setInterval(checkReminder, 60000); checkReminder(); }
function checkReminder(){ const now=new Date(); const start=parseTime(state.reminders.start); const end=parseTime(state.reminders.end); if(!within(now,start,end)) return; if(minutesSince(state.lastNotifyAt)>=state.reminders.intervalMin){ notify('¡Hey! 💧 Es hora de tomar agua'); } }
